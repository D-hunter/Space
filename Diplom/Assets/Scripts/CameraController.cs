...
public class CameraController : MonoBehaviour
{
		...
		void Start ()
		{
				this.transform.position = new Vector3 (0, 101, 0);
		...
		void Zoom ()
		{
				if ((Input.GetAxis ("Mouse ScrollWheel") < 0) && this.transform.position.y < 500) {
						this.transform.localPosition += new Vector3 (0, 10, 0);
						distance = transform.position.y;
				} else if ((Input.GetAxis ("Mouse ScrollWheel") > 0) && this.transform.position.y > 10) {
						this.transform.localPosition += new Vector3 (0, -10, 0);
						distance = transform.position.y;
				}
		}
